
    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null auto_increment,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id bigint not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer generated by default as identity,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id),
        constraint uq_coin_fav_user_coin unique (user_id, coin_id),
        constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id)
    );

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at timestamp(6) not null,
        id varchar(255) not null,
        primary key (id)
    );

    create table phantom_wallets (
        id bigint generated by default as identity,
        wallet_address varchar(255) not null unique,
        primary key (id)
    );

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    );

    create table user_session (
        created_at timestamp(6) not null,
        last_seen_at timestamp(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    );

    alter table if exists coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user;

    create table coin_favorites (
        id integer generated by default as identity,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id),
        constraint uq_coin_fav_user_coin unique (user_id, coin_id),
        constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id)
    );

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at timestamp(6) not null,
        id varchar(255) not null,
        primary key (id)
    );

    create table phantom_wallets (
        id bigint generated by default as identity,
        wallet_address varchar(255) not null unique,
        primary key (id)
    );

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    );

    create table user_session (
        created_at timestamp(6) not null,
        last_seen_at timestamp(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    );

    alter table if exists coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user;

    create table coin_favorites (
        id integer generated by default as identity,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id),
        constraint uq_coin_fav_user_coin unique (user_id, coin_id),
        constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id)
    );

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at timestamp(6) not null,
        id varchar(255) not null,
        primary key (id)
    );

    create table phantom_wallets (
        id bigint generated by default as identity,
        wallet_address varchar(255) not null unique,
        primary key (id)
    );

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    );

    create table user_session (
        created_at timestamp(6) not null,
        last_seen_at timestamp(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    );

    alter table if exists coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user;

    create table coin_favorites (
        id integer generated by default as identity,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id),
        constraint uq_coin_fav_user_coin unique (user_id, coin_id),
        constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id)
    );

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at timestamp(6) not null,
        id varchar(255) not null,
        primary key (id)
    );

    create table phantom_wallets (
        id bigint generated by default as identity,
        wallet_address varchar(255) not null unique,
        primary key (id)
    );

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    );

    create table user_session (
        created_at timestamp(6),
        last_seen_at timestamp(6),
        ip_address varchar(255),
        session_token_hash varchar(255) not null,
        user_agent varchar(255),
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (session_token_hash)
    );

    alter table if exists coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user;

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);

    create table coin_favorites (
        id integer not null auto_increment,
        coin_id varchar(255) not null,
        user_id varchar(255),
        wallet_address varchar(255),
        primary key (id)
    ) engine=InnoDB;

    create table coin_price (
        usd integer not null,
        usd_24h_change integer not null,
        usd_24h_vol integer not null,
        usd_market_cap integer not null,
        last_updated_at datetime(6) not null,
        id varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table phantom_wallets (
        id bigint not null auto_increment,
        wallet_address varchar(255) not null,
        primary key (id)
    ) engine=InnoDB;

    create table user (
        password_hash TEXT,
        user_id varchar(255) not null,
        user_role enum ('ADMIN','USER') not null,
        primary key (user_id)
    ) engine=InnoDB;

    create table user_session (
        created_at datetime(6) not null,
        last_seen_at datetime(6),
        user_id varchar(25),
        ip_address varchar(45),
        session_token_hash varchar(255) not null,
        user_agent TEXT,
        wallet_address varchar(255) not null,
        primary key (session_token_hash)
    ) engine=InnoDB;

    alter table coin_favorites 
       add constraint uq_coin_fav_user_coin unique (user_id, coin_id);

    alter table coin_favorites 
       add constraint uq_coin_fav_wallet_coin unique (wallet_address, coin_id);

    alter table phantom_wallets 
       add constraint UKosdpm734fvt17smm4v25otfaq unique (wallet_address);

    alter table coin_favorites 
       add constraint FKrblq4va1om43miqta5mmkvgce 
       foreign key (user_id) 
       references user (user_id);
